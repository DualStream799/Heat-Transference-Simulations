...
for t = 2:t_nodes
    ...
    for j = 2:y_nodes-1
        % Calculate the Temperature on the internal node:
        for i = 1:x_nodes-1
            if (i == 1)
                dTdx = 0;
                % Calculate the Central Diference on the X-axis:
                d2Tdx2 = ( T(i+1,j,t-1) - 2*T(i,j,t-1) + ( T(i+1,j,t-1) - 2*dx*dTdx ) )/ dx^2;
                % Calculate the Central Diference on the Y-axis:
                d2Tdy2 = ( T(i,j+1,t-1) - 2*T(i,j,t-1) + T(i,j-1,t-1) )/ dy^2;
                % Calculate the Temperature variation on the next node:
